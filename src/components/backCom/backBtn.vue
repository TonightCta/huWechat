<template>
  <div ref="icons" class="icons-container" :style="{left: left + 'px', top: top + 'px'}" @click="back()">
    <!-- <van-icon name="clear" size="30"/> -->
...
export default {
  ...
  mounted() {
    this.$nextTick(() => {
      // 获取滚动元素
      this.scrollContainer.addEventListener('scroll', this.handleScrollStart,{passive:false})
  ...
  methods: {
    back(){
      if(GetUrlKey('share_code',window.location.href)){
        window.location.replace('http://m.dahujing.com/test/?share_code='+GetUrlKey('share_code',window.location.href))
      }else{
        this.$router.back()
      }
    },
