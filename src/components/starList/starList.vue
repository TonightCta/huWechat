...
<template lang="html">
  <div class="starList">
    <p class="lisTitle">
      <van-icon name="aim" class="titleIcon"/>
      <slot></slot>
      <span>
        更多
...
export default {
  ...
  created(){
    if(this.homeStart.length>0){
      this.strList=this.homeStart
    }else{
      this.getStrList();
    }
  },
  ...
  methods:{
    ...
    getStrList(){//获取策略列表
      this.$axios.get(this.Uapi+'/user/strategy',{
        headers:{
          'v':'v2'
        }
      }).then((res)=>{
        if(res.data.state==1){
          ...
          this.homeStart_fn(res.data.data.list);
        }else{
